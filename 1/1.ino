...
void loop(){
  ...
  if (millis() - net_time1 > INTERVAL_SENSOR) //发送数据时间间隔
  {
    ...
    float hu_nan=(float)analogRead(A3);
    float sensor_hum = (float)DHT11.humidity;
    float sensor_tem = (float)DHT11.temperature;
    Serial.print("Humidity (%): ");
    Serial.println(sensor_hum, 3);
    Serial.print("intensity : ");
    Serial.println(hu_nan,3);

    Serial.print("Temperature (oC): ");
    Serial.println(sensor_tem, 3);
    Serial.println("");
    ...
    if (wifi.createTCP(HOST_NAME, HOST_PORT)) { //建立TCP连接，如果失败，不能发送该数据
      ...
      jsonToSend += ",\"intensity\":";
      dtostrf(hu_nan, 1, 2, buf);
      jsonToSend += "\"" + String(buf) + "\"";
